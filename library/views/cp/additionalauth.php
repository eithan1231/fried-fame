...
// https://eithan.me/
//
// \library\views\cp\additionalauth.php
//
// ======================================
...
<html lang="<?= $language->languageCode() ?>" dir="<?= $language->languageTextDirection() ?>" style="--sidebar-width: <?= $ff_config->get('sidebar-width') ?>">
	<head>
		<meta charset="utf-8">
		<title><?= ff_esc(ff_buildTitle('title-additional-auth', $language)) ?></title>
		...
		<div class="container" style="padding-top: 10px;">
			<div class="card card-info">
				<div class="card-header">
					<h4><?= ff_esc($language->getPhrase($formRaw['title'])) ?></h4>
				</div>
				<div class="card-body">
						...
						<?php foreach($formRaw['input'] as $key => $value): ?>
							<div class="input-group mb-3">
								<div class="input-group-prepend">
									<span class="input-group-text" style="width: 135px"><?= ff_esc($language->getPhrase($value['label'])) ?></span>
								</div>
								<input name="<?= ff_esc($key) ?>" type="<?= ff_esc($value['type']) ?>" class="form-control" placeholder="<?= ff_esc($language->getPhrase($value['label'])) ?>"autofocus>
							</div>
						<?php endforeach; ?>

						<?php if($formRaw['requireCaptcha']): ?>
