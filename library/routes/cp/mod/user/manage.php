...
// https://eithan.me/
//
// \library\routes\cp\mod\user\manage.php
//
// ======================================


class routes_cp_mod_user_manage extends route
{
	/**
	...
	public function run(array $parameters, request &$request, response &$response)
	{
      ...
      return true;
		...
		if(!$request->get('user') && !is_numeric($request->get('user'))) {
			// Will treat page as not found, which technically it isn't.
			return false;
		}

		if(!$ff_context->getSession()->getActiveLinkUser()->getGroup()->can('mod_users')) {
